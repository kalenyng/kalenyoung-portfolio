...
export interface LineWavesHandle {
  dispose: () => void;
}
...
export function createLineWaves(
  container: HTMLElement,
  options: LineWavesOptions = {}
): LineWavesHandle | null {
  const {
    speed = 0.3,
    innerLineCount = 32,
    outerLineCount = 36,
  ...
  const update = (time: number) => {
    ...
    program.uniforms.uTime.value = reduced ? 0 : time * 0.001;
